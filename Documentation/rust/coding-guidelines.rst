...
``rustfmt``, by default, formats import statements in a way that are prone to
conflicts while merging and rebasing, since in some cases it condenses several
items into the same line. For instance:
...
``rustfmt`` will actually reformat import statements vertically when the empty
comment is added. That is, it is possible to easily reformat the original
example into the expected style by running ``rustfmt`` on an input like:
...
for single item import statements -- this can be useful to minimize diffs within
patch series:
...
trailing comma in other formatters. Sometimes it may be simpler to avoid to move
...
a hard rule, and there is also code that is not migrated to this style yet, but
